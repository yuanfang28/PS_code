...
#                    4.2, 0.4, 0.7,  0.5 , np.pi/6,
#                    6.5, 0.4, 0.6, 0.6, np.pi/2,
#                    9, -0.4, 0.7, 0.3, np.pi/4,
#                    11, -0.4, 1, 0.5, 0,
#                    13, -0.4, 0.7, 0.4, np.pi/3,])

#障碍物例子5，多个障碍物,交叉集中在两侧
# obs_val = np.array([2.3, 0.4, 0.5, 0.5,0,
...
for i in range(70):
    ...
    #if i == 49:
        #ax.plot(x_plot[-1], f_values[-1], 'ro', markersize=2, label="Endpoint of each step")  # 'ro' 表示红色圆点，markersize调整圆点大小  # 'ro' 表示红色圆点，markersize调整圆点大小
        #ax.plot(f_up_x_values[-1], f_up_y_values[-1], 'ro', markersize=2)
        #ax.plot(f_down_x_values[-1], f_down_y_values[-1], 'ro', markersize=2)
    #else:
        #ax.plot(x_plot[-1], f_values[-1], 'ro', markersize=2)
        #ax.plot(f_up_x_values[-1], f_up_y_values[-1], 'ro', markersize=2)
        #ax.plot(f_down_x_values[-1], f_down_y_values[-1], 'ro', markersize=2)
...
#cbar = plt.colorbar(sm, ax=ax, orientation='horizontal', pad=0.2,shrink=0.8)
